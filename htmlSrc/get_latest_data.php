...
if ($potID > 0) {
    // Prepare a statement to fetch the last 10 data points for the specified pot
    $stmt = $conn->prepare("SELECT HUMIDITY, TEMPERATURE, DATE_FORMAT(TIMESTAMP, '%Y-%m-%dT%H:%i:%s') AS TIMESTAMP FROM sensor_data WHERE POT_ID = ? ORDER BY TIMESTAMP DESC LIMIT 30");
    if (!$stmt) {
